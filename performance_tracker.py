...
""".strip()
    ...
    async def post_daily_recap(self):
        """Post a 24-hour performance recap"""
        try:
            # Get outcomes from last 24 hours
            outcomes = await self.db.get_outcomes(days=1)
            
            if not outcomes:
                logger.info("No outcomes to recap")
                return
            ...
            # Get signals that hit certain thresholds
            signals_2x = [o for o in outcomes if o.get('peak_gain', 0) >= 100]
            signals_10pct = [o for o in outcomes if o.get('peak_gain', 0) >= 10]
            
            # Calculate ROI stats
            gains = [o.get('peak_gain', 0) for o in outcomes if o.get('peak_gain', 0) > 0]
            median_roi = sorted(gains)[len(gains)//2] if gains else 0
            max_roi = max(gains) if gains else 0
            
            # Get top performers (top 5)
            top_performers = sorted(
                outcomes,
                key=lambda x: x.get('peak_gain', 0),
                reverse=True
            )[:5]
            
            # Build recap message
            message = f"""📊 **SENTINEL SIGNALS - 24H RECAP** 🎯
...
📊 Median ROI: {median_roi/100:.1f}x
🏆 Max ROI: {max_roi/100:.1f}x
📢 Total Signals: {total_signals} {"🔥" if total_signals >= 5 else ""}
✅ Signals ≥2x: {len(signals_2x)} ({len(signals_2x)/total_signals*100:.1f}% hit rate) {"🚀" if len(signals_2x)/total_signals >= 0.5 else ""}
✅ Signals ≥10%: {len(signals_10pct)} ({len(signals_10pct)/total_signals*100:.1f}%)
...
"""
            ...
            for perf in top_performers:
                multiplier = (perf.get('peak_gain', 0) / 100) + 1
                message += f"[{multiplier:.1f}x] ${perf['symbol']}\n"
            
            message += "\nKeep following for more alpha! 🎯"
            
            await self.telegram.send_message(message)
            logger.success("✓ Posted daily recap")
